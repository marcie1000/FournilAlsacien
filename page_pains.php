...
<html>
    <body>
        ...
        for ($i = 1; $i <= $nb; $i++) {
            ...
            $sql = $pdo->prepare('SELECT ALLERGENE.denomination, EXISTER.presence, EXISTER.trace FROM ALLERGENE, EXISTER WHERE EXISTER.refP = "P00'.$i.'" AND ALLERGENE.id = EXISTER.id;');
            $sql->execute();
